...
#define handle_error_en(en, msg) \
   do { errno = en; perror(msg); exit(errno);} while (0)
...
int CreateAndArmTimer(int unsigned period, ThreadArgs* info)
{
   ...
   if(nanoseconds > 999999999)
   {
      ...
      /*
        ...
        I implemented this check to explain that issue.
      */
   }
   ...
   if(ret || ret2 || ret3 || ret4)
   {
     ...
     printf("========================\n");
      if(ret)
      {
         printf("Sig Empty Set: %d\n",ret);
      }
      if(ret2)
      {
         printf("Sig Add Set: %d\n", ret2);
      }
      if(ret3)
      {
...
void* threadFunction(void *arg)
{
   ...
   if(retVal)
   {
      handle_error_en(ENOSYS, "Create and Arm Timer");
   }
