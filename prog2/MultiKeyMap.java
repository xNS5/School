...
public class MultiKeyMap<V> {
    ...
    //  update
    //Pre-conditions:
    //  - k1 and k2 are two keys, neither containing the | symbol.
    //  - value is the new value which you want to store
    //Post-conditions:
    //  - {k1,k2} now maps to the updated value in MultiKeyMap


     public void update(String k1, String k2, V value)
      {
        if( k1.contains("|") || k2.contains("|") ) {
            System.err.println("Error: Keys in a MultiKeyMap cannot contain the bar character (\"|\")");
            System.exit(10);
          }
        V next = map.get(k1 + "|" + k2);
        if(next != null)
          {
            map.put(k1 + "|" + k2, next);
          }
